...
class MoGeModel(nn.Module):
        ...
        **deprecated_kwargs,
        ...
        self.head = Head(
            ...
            dim_out=(
                3
                if not output_mask
                else 4 if output_mask and not split_head else [3, 1]
            ),
            dim_proj=dim_proj,
    ...
    def forward(
        self, image: torch.Tensor, mixed_precision: bool = False
    ...
    def infer(
        ...
        if fov_x is None:
            focal, shift = recover_focal_shift(
                points, None if mask is None else mask > 0.5
            )
        else:
            focal = (
                aspect_ratio
            ...
            _, shift = recover_focal_shift(
                points, None if mask is None else mask > 0.5, focal=focal
            )
