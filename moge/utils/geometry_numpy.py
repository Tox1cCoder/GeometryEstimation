...
def image_plane_uv_numpy(width: int, height: int, aspect_ratio: float = None,
...
def solve_optimal_shift_focal(uv: np.ndarray, xyz: np.ndarray, ransac_iters: int = None,
                              ransac_hypothetical_size: float = 0.1, ransac_threshold: float = 0.1):
    ...
    if ransac_iters is None:
        ...
        for _ in range(ransac_iters):
            ...
            solution = least_squares(partial(fn, uv[maybe_inliers], xy[maybe_inliers], z[maybe_inliers]),
                                     x0=initial_shift, ftol=1e-3, method='lm')
            ...
            if confirmed_inliers.sum() > 10:
                solution = least_squares(
                    partial(fn, uv[confirmed_inliers], xy[confirmed_inliers], z[confirmed_inliers]), x0=maybe_shift,
                    ftol=1e-3, method='lm')
