...
def download_blob_with_cache(container: Union[str, ContainerClient], blob_name: str,
                             cache_dir: str = 'blobcache') -> bytes:
    """
...
def open_blob_with_cache(container: Union[str, ContainerClient], blob_name: str,
                         cache_dir: str = 'blobcache') -> io.BytesIO:
    """
    Open a blob file for reading from a container and return its content as a BytesIO object.
    If the file is already present in the cache, it is read from there.
    """
...
class AzureBlobStringWriter(io.StringIO):
    ...
    def close(self):
        self.blob_client.upload_blob(self.getvalue().encode(self._encoding), blob_type='BlockBlob', overwrite=True,
                                     **self.kwargs)
...
def open_azure_blob(blob: Union[str, BlobClient], mode: str = 'r', encoding: str = 'utf-8', newline: str = None,
                    cache_blob: bool = False, **kwargs) -> IO:
    if isinstance(blob, str):
        blob_client = BlobClient.from_blob_url(blob)
...
class AzureBlobPath(PurePosixPath):
    ...
    def __init__(self, root: Union[str, 'AzureBlobPath', ContainerClient], *others: Union[str, PurePosixPath],
                 pool_maxsize: int = 256, retries: int = 3):
        if isinstance(root, AzureBlobPath):
            ...
            if sas:
                ...
                self.container_client = ContainerClient.from_container_url(container, credential=DEFAULT_CREDENTIAL,
                                                                           session=session)
            parts = (path, *others)
