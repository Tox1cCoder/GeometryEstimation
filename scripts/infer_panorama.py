...
def get_panorama_cameras():
    ...
    intrinsics = utils3d.numpy.intrinsics_from_fov(
        fov_x=np.deg2rad(90), fov_y=np.deg2rad(90)
    )
    extrinsics = utils3d.numpy.extrinsics_look_at(
        [0, 0, 0], vertices, [0, 0, 1]
    ).astype(np.float32)
...
def split_panorama_image(
    image: np.ndarray, extrinsics: np.ndarray, intrinsics: np.ndarray, resolution: int
):
    ...
    for i in range(len(extrinsics)):
        spherical_uv = directions_to_spherical_uv(
            utils3d.numpy.unproject_cv(
                uv, extrinsics=extrinsics[i], intrinsics=intrinsics[i]
            )
        )
        pixels = utils3d.numpy.uv_to_pixel(
            spherical_uv, width=width, height=height
        ).astype(np.float32)

        splited_image = cv2.remap(
            image, pixels[..., 0], pixels[..., 1], interpolation=cv2.INTER_LINEAR
        )
...
def poisson_equation(
    ...
    data = (
        np.array([[-4, 1, 1, 1, 1]], dtype=np.float32)
        .repeat(height * width, axis=0)
        .reshape(-1)
    )
    indices = np.stack(
        [
...
def grad_equation(
    ...
    data = np.concatenate(
        [
            np.concatenate(
                [
                    np.ones(
                        (grid_index.shape[0], grid_index.shape[1] - 1), dtype=np.float32
                    ).reshape(
                        -1, 1
                    ),  # x[i,j]
                    -np.ones(
            ...
            np.concatenate(
                [
                    np.ones(
                        (grid_index.shape[0] - 1, grid_index.shape[1]), dtype=np.float32
                    ).reshape(
                        -1, 1
                    ),  # x[i,j]
                    -np.ones(
    ...
    indptr = np.arange(
        0,
        grid_index.shape[0] * (grid_index.shape[1] - 1) * 2
        + (grid_index.shape[0] - 1) * grid_index.shape[1] * 2
        + 1,
        2,
    )
    A = csr_array(
        (data, indices, indptr),
        shape=(
            grid_index.shape[0] * (grid_index.shape[1] - 1)
            + (grid_index.shape[0] - 1) * grid_index.shape[1],
            height * width,
        ),
    )
...
def merge_panorama_depth(
    ...
    if max(width, height) > 256:
        ...
        panorama_depth_init = cv2.resize(
            panorama_depth_init, (width, height), cv2.INTER_LINEAR
        )
    else:
    ...
    for i in range(len(distance_maps)):
        ...
        projected_pixels = utils3d.numpy.uv_to_pixel(
            ...
            width=distance_maps[i].shape[1],
            height=distance_maps[i].shape[0],
        ).astype(np.float32)

        log_splited_distance = np.log(distance_maps[i])
        panorama_log_distance_map = np.where(
            projection_valid_mask,
            cv2.remap(
                log_splited_distance,
                projected_pixels[..., 0],
                projected_pixels[..., 1],
                cv2.INTER_LINEAR,
        ...
        laplacian = convolve(
            padded, np.array([[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=np.float32)
        )[1:-1, 1:-1]
    ...
    panorama_log_distance_grad_x = np.stack(
        [grad_map[0] for grad_map in panorama_log_distance_grad_maps], axis=0
    )
    panorama_log_distance_grad_y = np.stack(
        [grad_map[1] for grad_map in panorama_log_distance_grad_maps], axis=0
    )
    panorama_grad_mask_x = np.stack(
        [mask_map[0] for mask_map in panorama_grad_masks], axis=0
    )
    panorama_grad_mask_y = np.stack(
        [mask_map[1] for mask_map in panorama_grad_masks], axis=0
    )

    panorama_log_distance_grad_x = np.sum(
        panorama_log_distance_grad_x * panorama_grad_mask_x, axis=0
    ) / np.sum(panorama_grad_mask_x, axis=0).clip(1e-3)
    panorama_log_distance_grad_y = np.sum(
        panorama_log_distance_grad_y * panorama_grad_mask_y, axis=0
    ) / np.sum(panorama_grad_mask_y, axis=0).clip(1e-3)
    ...
    panorama_laplacian_map = np.sum(
        panorama_laplacian_maps * panorama_laplacian_masks, axis=0
    ) / np.sum(panorama_laplacian_masks, axis=0).clip(1e-3)
    ...
    x, *_ = lsmr(
        ...
        btol=1e-5,
        x0=(
            np.log(panorama_depth_init).reshape(-1)
            if panorama_depth_init is not None
            else None
        ),
        show=False,
    )
...
@click.option(
    ...
    help='Pretrained model name or path. Default is "Ruicheng/moge-vitl"',
)
@click.option(
    "--device",
    "device_name",
    type=str,
    default="cuda",
    help='Device name (e.g. "cuda", "cuda:0", "cpu"). Default is "cuda"',
)
@click.option(
    "--resize",
    "resize_to",
    type=int,
    default=None,
    help="Resize the image(s) & output maps to a specific size. Default is None (no resizing).",
)
@click.option(
    "--resolution_level",
    type=int,
    default=9,
    help="An integer [0-9] for the resolution level of inference. The higher, the better but slower. Default is 9. Note that it is irrelevant to the output resolution.",
)
@click.option(
    "--threshold",
    type=float,
    default=0.03,
    help='Threshold for removing edges. Default is 0.03. Smaller value removes more edges. "inf" means no thresholding.',
)
@click.option(
    "--batch_size", type=int, default=4, help="Batch size for inference. Default is 4."
)
@click.option(
    "--splited",
    "save_splited",
    is_flag=True,
    help="Whether to save the splited images. Default is False.",
)
@click.option(
    "--maps",
    "save_maps_",
    is_flag=True,
    help="Whether to save the output maps and fov(image, depth, mask, points, fov).",
)
@click.option(
    "--glb",
    "save_glb_",
    is_flag=True,
    help="Whether to save the output as a.glb file. The color will be saved as a texture.",
)
@click.option(
    "--ply",
    "save_ply_",
    is_flag=True,
    help="Whether to save the output as a.ply file. The color will be saved as vertex colors.",
)
@click.option(
    "--show",
    "show",
    is_flag=True,
    help="Whether show the output in a window. Note that this requires pyglet<2 installed as required by trimesh.",
)
def main(
    input_path: str,
    output_path: str,
    ...
    if Path(input_path).is_dir():
        image_paths = sorted(
            itertools.chain(
                *(Path(input_path).rglob(f"*.{suffix}") for suffix in include_suffices)
            )
        )
    else:
    ...
    if not any([save_maps_, save_glb_, save_ply_]):
        warnings.warn(
            'No output format specified. Please use "--maps", "--glb", or "--ply" to specify the output.'
        )

    model = MoGeModel.from_pretrained(pretrained_model_name_or_path).to(device).eval()

    for image_path in (
        pbar := tqdm(image_paths, desc="Total images", disable=len(image_paths) <= 1)
    ):
        image = cv2.cvtColor(cv2.imread(str(image_path)), cv2.COLOR_BGR2RGB)
        height, width = image.shape[:2]
        if resize_to is not None:
            height, width = min(resize_to, int(resize_to * height / width)), min(
                resize_to, int(resize_to * width / height)
            )
            image = cv2.resize(image, (width, height), cv2.INTER_AREA)

        splited_extrinsics, splited_intriniscs = get_panorama_cameras()
        splited_resolution = 512
        splited_images = split_panorama_image(
            image, splited_extrinsics, splited_intriniscs, splited_resolution
        )
        ...
        for i in trange(
            ...
            len(splited_images),
            batch_size,
            desc="Inferring splited views",
            disable=len(splited_images) <= batch_size,
            leave=False,
        ):
            image_tensor = torch.tensor(
                np.stack(splited_images[i : i + batch_size]) / 255,
                dtype=torch.float32,
                device=device,
            ).permute(0, 3, 1, 2)
            fov_x, fov_y = np.rad2deg(
                utils3d.numpy.intrinsics_to_fov(
                    np.array(splited_intriniscs[i : i + batch_size])
                )
            )
            fov_x = torch.tensor(fov_x, dtype=torch.float32, device=device)
            output = model.infer(image_tensor, fov_x=fov_x, apply_mask=False)
            distance_map, mask = (
                output["points"].norm(dim=-1).cpu().numpy(),
        ...
        if save_splited:
            ...
            for i in range(len(splited_images)):
                cv2.imwrite(
                    str(splited_save_path / f"{i:02d}.jpg"),
                    cv2.cvtColor(splited_images[i], cv2.COLOR_RGB2BGR),
                )
                cv2.imwrite(
                    str(splited_save_path / f"{i:02d}_distance_vis.png"),
                    cv2.cvtColor(
                        colorize_depth(splited_distance_maps[i], splited_masks[i]),
                        cv2.COLOR_RGB2BGR,
                    ),
                )
        ...
        points = panorama_depth[:, :, None] * spherical_uv_to_directions(
            utils3d.numpy.image_uv(width=width, height=height)
        )

        # Write outputs
        save_path = Path(
            output_path, image_path.relative_to(input_path).parent, image_path.stem
        )
        save_path.mkdir(exist_ok=True, parents=True)
        if save_maps_:
            cv2.imwrite(
                str(save_path / "image.jpg"), cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            )
            cv2.imwrite(
                str(save_path / "depth.png"),
                cv2.cvtColor(colorize_depth(panorama_depth), cv2.COLOR_RGB2BGR),
            )
            cv2.imwrite(
