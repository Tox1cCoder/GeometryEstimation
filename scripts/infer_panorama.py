...
@click.option(
    ...
    "save_splitted",
    is_flag=True,
    help="Whether to save the splitted images. Default is False.",
)
@click.option(
    "--maps",
    "save_maps_",
...
def main(
    ...
    for image_path in (
        ...
        for i in trange(
            ...
            len(splitted_images),
            batch_size,
            desc="Inferring splitted views",
            disable=len(splitted_images) <= batch_size,
            leave=False,
        ):
            image_tensor = torch.tensor(
                np.stack(splitted_images[i : i + batch_size]) / 255,
                dtype=torch.float32,
                device=device,
            ).permute(0, 3, 1, 2)
            fov_x, fov_y = np.rad2deg(
                utils3d.numpy.intrinsics_to_fov(
                    np.array(splitted_intriniscs[i : i + batch_size])
                )
            )
        ...
        if save_splitted:
            ...
            for i in range(len(splitted_images)):
                cv2.imwrite(
                    str(splitted_save_path / f"{i:02d}.jpg"),
                    cv2.cvtColor(splitted_images[i], cv2.COLOR_RGB2BGR),
                )
                cv2.imwrite(
                    str(splitted_save_path / f"{i:02d}_distance_vis.png"),
                    cv2.cvtColor(
                        colorize_depth(splitted_distance_maps[i], splitted_masks[i]),
                        cv2.COLOR_RGB2BGR,
                    ),
                )
        ...
        panorama_mask = (
            cv2.resize(
                panorama_mask.astype(np.uint8), (width, height), cv2.INTER_NEAREST
            )
            > 0
        )
        ...
        (
            ...
            if pbar.disable
            else pbar.set_postfix_str(f"Inferring")
        )
        save_path = Path(
        ...
        if save_maps_:
            ...
            cv2.imwrite(
                str(save_path / "depth_vis.png"),
                cv2.cvtColor(
                    colorize_depth(panorama_depth, mask=panorama_mask),
                    cv2.COLOR_RGB2BGR,
                ),
            )
            cv2.imwrite(
